...
{
  NS_LOG_FUNCTION (this);
  m_dcf->StartBackoffNow (m_rng->GetNext (0, m_dcf->GetCw ()), 99); // "99" added for logging (m_startBackoffCountEdca) ***
  ...
  if (!NeedRtsRetransmission ())
    {
      ...
      m_dcf->ResetCw (99); // "99" added for logging (m_cwResetCountEdca) ***
    }
  else
    {
      m_dcf->UpdateFailedCw (3); // "3" added for logging (m_cwFailedCountEdcaMissedCts) ***
    }
  m_dcf->StartBackoffNow (m_rng->GetNext (0, m_dcf->GetCw ()), 99); // "99" added for logging (m_startBackoffCountEdca) ***
  RestartAccessIfNeeded ();
}
...
EdcaTxopN::MissedAck (void)
{
  ...
  if (!NeedDataRetransmission ())
  ...
  else
    {
      ...
      m_dcf->UpdateFailedCw (4); // "4" added for logging (m_cwFailedCountEdcaMissedAck) ***
    }
  m_dcf->StartBackoffNow (m_rng->GetNext (0, m_dcf->GetCw ()), 99); // "99" added for logging (m_startBackoffCountEdca) ***
  RestartAccessIfNeeded ();
}
  ...
  if (NeedBarRetransmission ())
    {
      ...
      m_dcf->UpdateFailedCw (5); // "5" added for logging (m_cwFailedCountEdcaMissedBlockAck) ***
    }
  else
    {
