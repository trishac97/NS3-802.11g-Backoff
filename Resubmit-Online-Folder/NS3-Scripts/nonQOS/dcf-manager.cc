...
DcfState::UpdateFailedCw (uint32_t logVal) // *** (Variation of ::UpdateFailedCw for the logVal) ***
{
  ...
  if(m_backoffType == 7) // *** MY: Truncated Sawtooth Backoff xyz***
  {
    //double firstlog = 0.0;
    //double secondlog = 0.0;
    //int constant_trunc = 0.5;  

    if(m_sawtoothCount > 0) // we go to the truncatedBound
    {
        m_cw = std::min (m_cw/2, m_cwMax); // ***
        m_sawtoothCount -= 1; // ***
        //truncatedBound = truncatedBound - 1;
        //std::cout << "----------- m_sawtoothCount = " << m_sawtoothCount << "-------------------SUB-Window = " << m_cw << "\n";
    }
    
    else // *** m_sawtoothCount has reached c*loglog n iterations ***
    {
         m_sawtoothRound += 1; // ***
         m_cw = std::min ((uint32_t) pow(2,m_sawtoothRound), m_cwMax); // set the window
         ...
         //m_sawtoothCount = std::min( (uint32_t)m_sawtoothRound ,  ((uint32_t)(floor(constant_trunc*secondlog) + 1) )); 
          //m_sawtoothCount = 0;      
        m_sawtoothCount = std::min( (uint32_t)m_sawtoothRound ,  ((uint32_t)100));
        // std::cout << "---------- m_sawtoothCount = " << m_sawtoothCount << "------ m_sawtoothRound = " <<  m_sawtoothRound << "------------Window Increase = " << m_cw << "\n";
    }  
